...
export default function DetailView({ couponId, coupon, chartRange, setChartRange, onEdit, onBack }) {
    ...
    const handleDuplicate = () => {
        ...
        const duplicatedCoupon = {
            ...
            // Reset statistics
            usageCount: 0, discountGiven: 0, revenueInfluenced: 0, newCustomers: 0,
            chartData: [], rejectionReasons: [], redemptionHistory: []
        };
        onEdit(duplicatedCoupon);
    };
    ...
    const chartData = data.chartData || [];
    const rejectionReasons = data.rejectionReasons || [];
    const redemptionHistory = data.redemptionHistory || data.orders || []; // Fallback mapping array
            ...
            <div className="detail-grid">
                <div>
                    <div className="card chart">
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div>
                                <div style={{ fontSize: 14.5, fontWeight: 600 }}>
                                    {chartRange === 'weekly' ? 'Weekly redemptions' : 'Daily redemptions'}
                                </div>
                                <div className="sub" style={{ fontSize: 12.5 }}>Real-time activity</div>
                            </div>
                            <div className="seg">
                        ...
                        {(() => {
                            const activeChartData = chartRange === 'weekly'
                                ? (data.weeklyChartData || [])
                                : (data.dailyChartData || data.chartData || []);

                            return activeChartData.length > 0 ? (
                                <div style={{ marginTop: '20px' }}>
                                    <div className="chart-bars" style={{ display: 'flex', alignItems: 'flex-end', height: '140px', gap: '8px', paddingBottom: '8px' }}>
                                        {activeChartData.map((b, i) => (
                                            <div key={i} style={{ flex: 1, height: `${Math.max(10, b.outer)}%`, position: 'relative', background: 'var(--primary-wash)', borderRadius: '3px 3px 0 0' }}>
                                                <i style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: `${b.inner}%`, background: 'var(--primary)', borderRadius: '3px 3px 0 0', display: 'block' }} />
                                            </div>
                                        ))}
                                    </div>
                                    {/* Bottom Date Labels Row */}
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--muted)', marginTop: '8px', borderTop: '1px solid var(--border)', paddingTop: '6px' }}>
                                        <span>{activeChartData[0]?.label || ''}</span>
                                        <span>{activeChartData[Math.floor(activeChartData.length / 2)]?.label || ''}</span>
                                        <span>{activeChartData[activeChartData.length - 1]?.label || ''}</span>
                                    </div>
                                </div>
                            ) : (
                                <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>
                        ...
                        <div className="legend" style={{ marginTop: '16px' }}>
                            <span><i style={{ background: 'var(--primary)', display: 'inline-block', width: '10px', height: '10px', marginRight: '4px' }} />Redeemed</span>
                            <span><i style={{ background: 'var(--primary-wash)', display: 'inline-block', width: '10px', height: '10px', marginRight: '4px' }} />Applied but not checked out</span>
                        </div>
                    </div>

                    {/* --- NEW: COUPON REDEMPTION & USAGE LEDGER --- */}
                    <div className="card panel" style={{ marginTop: 16 }}>
                        <h2 className="panel-h">Coupon Redemption Ledger</h2>
                        <p className="panel-s">Detailed list of users, amounts, and orders where this coupon was applied successfully.</p>
                        <table>
                            <thead>
                            <tr>
                                <th>Customer</th>
                            ...
                            {redemptionHistory.length > 0 ? (
                                redemptionHistory.map((item, i) => (
                                    <tr key={i}>
                                        <td>
                                            <div style={{ fontWeight: 600 }}>{item.userName || item.name || 'Guest User'}</div>
                                            <div style={{ fontSize: '11.5px', color: 'var(--muted)' }}>{item.userEmail || item.email || 'N/A'}</div>
                                        </td>
                                        <td>
                                        ...
                                        <td className="num" style={{ fontWeight: 600, color: '#28a745' }}>
                                            -৳{(item.discountAmount || item.couponDiscountAmount || 0).toFixed(2)}
                                        </td>
                                        <td style={{ fontSize: '12.5px', color: 'var(--muted)' }}>
                                            {formatDateString(item.createdAt || item.issuedAt)}
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td colSpan="4" style={{ textAlign: 'center', padding: '30px 0', color: 'var(--muted)', fontSize: 13 }}>
                                        No usage redemptions found for this coupon yet.
                                    </td>
                                </tr>
                            )}
                            </tbody>
