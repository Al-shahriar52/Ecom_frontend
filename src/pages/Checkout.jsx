...
const Checkout = () => {
    const { cart, cartTotal, fetchCart } = useContext(CartContext);
    // 1. EXTRACT isGuest FROM AUTH CONTEXT
    ...
    // --- 2. INITIALIZE USER DATA (Updated to bypass dummy guest profiles) ---
    useEffect(() => {
        // Only pre-fill and lock fields if a user exists AND they are NOT a guest shopper
        if (user && !isGuest) {
            const loginIdentifier = user.name || user.username || "";
            ...
            setFormData(prev => ({
                ...
                email: isEmail ? loginIdentifier : '',
                phone: !isEmail ? loginIdentifier : '',
                name: user.realName || '' // Maps their real name if your backend provides it
            }));

            // Lock the field they used to register/login
            setLockedField(isEmail ? 'email' : 'phone');
        } else {
            // If they are a guest session or completely anonymous, reset fields and keep everything unlocked
            setFormData(prev => ({
                ...prev,
                name: '',
                phone: '',
