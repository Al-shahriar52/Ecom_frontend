...
import {
    X, UserPlus, UserCog, UploadCloud, FileText, AlertCircle, User, Phone, Loader2
} from "lucide-react";
import { usePermissions } from "../../../context/PermissionContext";
import { ModalShell } from "./Shared";
import axiosInstance from "../../../api/AxiosInstance";

const API_ENDPOINT = "/api/v1/admin/users";
...
export default function UserFormModal({ user, onClose, onSuccess }) {
    ...
    // --- State Management ---
    const [errors, setErrors] = useState({ email: "", phone: "", global: "" });
    const [file, setFile] = useState(null);
    ...
    const handleChange = (e) => {
        ...
        if (name === "phone") {
            setErrors(prev => ({
                ...prev,
                phone: value && !validatePhone(value) ? "Please enter a valid Bangladeshi phone number." : ""
            }));
        }
    };
    ...
    const handleFileChange = (e) => {
        ...
        if (selected && selected.type !== "text/csv" && !selected.name.endsWith(".csv")) {
            setErrors(prev => ({ ...prev, global: "Please upload a valid CSV file." }));
            setFile(null);
            return;
        }
        setFile(selected);
    ...
    // --- Single User Create/Update using Axios ---
    const handleSingleUserSubmit = async () => {
        let emailErr = !validateEmail(formData.email) ? "Please enter a valid email address." : "";
        let phoneErr = formData.phone && !validatePhone(formData.phone) ? "Please enter a valid Bangladeshi phone number." : "";

        if (emailErr || phoneErr) {
            setErrors(prev => ({ ...prev, email: emailErr, phone: phoneErr }));
            return;
        }
        ...
        try {
            if (isEditMode) {
                await axiosInstance.put(`${API_ENDPOINT}/${user.id}`, payload);
                toast.success("User updated successfully!");
            } else {
                await axiosInstance.post(API_ENDPOINT, payload);
                toast.success("User created! Temporary password sent to email.");
            }

            if (onSuccess) onSuccess();
            onClose();
        } catch (err) {
            const apiMessage = err.response?.data?.message || "Failed to save user details.";
            setErrors(prev => ({ ...prev, global: apiMessage }));
    ...
    // --- Bulk Import CSV using Axios Multipart ---
    const handleBulkImportSubmit = async () => {
        ...
        try {
            const response = await axiosInstance.post(`${API_ENDPOINT}/bulk-import`, formData, {
                headers: {
            ...
            if (result.errorCount === 0) {
                ...
                toast.error(`Imported with ${result.errorCount} error(s). Review details below.`);
            }
        } catch (err) {
            const apiMessage = err.response?.data?.message || "Failed to process bulk import.";
            setErrors(prev => ({ ...prev, global: apiMessage }));
            toast.error(apiMessage);
        } finally {
            setIsSubmitting(false);
    ...
    return (
        <ModalShell onClose={onClose} size="md">
            <div className="um-modal-header-bordered">
                ...
                {!isEditMode && (
                    <div style={{ display: 'flex', gap: '8px', marginTop: '16px', padding: '0 20px 16px' }}>
                        <button
                            type="button"
                            onClick={() => { setEntryMode("single"); setErrors({ email: "", phone: "", global: "" }); }}
                            style={{
                                flex: 1, padding: '8px', borderRadius: '6px', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                                border: entryMode === "single" ? '1px solid var(--um-brand)' : '1px solid var(--um-border)',
                                background: entryMode === "single" ? 'var(--um-brand-bg)' : 'transparent',
                        ...
                        <button
                            type="button"
                            onClick={() => { setEntryMode("bulk"); setErrors({ email: "", phone: "", global: "" }); }}
                            style={{
                                flex: 1, padding: '8px', borderRadius: '6px', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                                border: entryMode === "bulk" ? '1px solid var(--um-brand)' : '1px solid var(--um-border)',
                                background: entryMode === "bulk" ? 'var(--um-brand-bg)' : 'transparent',
            ...
            <form onSubmit={handleSubmit}>
                <div className="um-modal-body">
                    {errors.global && (
                        <div style={{ marginBottom: '16px', padding: '12px', borderRadius: '6px', backgroundColor: 'var(--um-red-bg)', color: 'var(--um-red-text)', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
                            <AlertCircle size={16} />
                            {errors.global}
                        </div>
                    )}
                    ...
                    {entryMode === "single" && (
                        <>
                            ...
                            {!isEditMode && (
                                <div className="um-note-dashed">
                                    <p className="um-note-text">
                                        <strong>Note:</strong> The user will receive an email containing their generated temporary password.
                                    </p>
                                </div>
                            )}
                        </>
                    ...
                    {entryMode === "bulk" && (
                        <>
                            <div style={{
                                ...
                                {!file ? (
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                                        <UploadCloud size={32} color="var(--um-muted)" />
                                        <p className="um-text-ink" style={{ fontWeight: 500, margin: 0 }}>Click to upload or drag and drop</p>
                                        <p className="um-text-muted" style={{ fontSize: '13px', margin: 0 }}>CSV files only. Format: name, email, phone, role, gender</p>
                                    </div>
                                ) : (
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                                        <FileText size={32} color="var(--um-brand)" />
                            ...
                            {bulkResponse && (
                                <div style={{ marginTop: '16px', padding: '12px', borderRadius: '6px', backgroundColor: 'var(--um-surface)', border: '1px solid var(--um-border)', fontSize: '13px' }}>
                                    <p style={{ margin: '0 0 6px 0', fontWeight: 600 }}>
                                        Import Complete: {bulkResponse.successCount} succeeded, {bulkResponse.errorCount} failed out of {bulkResponse.totalProcessed}.
                                    </p>
                                    {bulkResponse.errors?.length > 0 && (
                                        <ul style={{ margin: 0, paddingLeft: '18px', color: 'var(--um-red)' }}>
                            ...
                            <div className="um-note-dashed" style={{ marginTop: '16px' }}>
                                <p className="um-note-text">
                                    <strong>Expected CSV Headers:</strong> <code>name, email, phone, role, gender</code>
                                </p>
                            </div>
                        </>
                    )}
                ...
                <div className="um-modal-footer">
                    <button type="button" onClick={onClose} className="um-btn-ghost" disabled={isSubmitting}>
                        Cancel
                    </button>
                    ...
                    <button
                        ...
                        disabled={
                            isSubmitting ||
                            (entryMode === "single" && (!formData.name || !formData.email || !!errors.email || !!errors.phone)) ||
                            (entryMode === "bulk" && !file)
                        }
                    >
                        {isSubmitting ? (
                            ...
                            entryMode === "single" ? (isEditMode ? "Save Changes" : "Create User") : "Import Users"
                        )}
                    </button>
                </div>
            </form>
